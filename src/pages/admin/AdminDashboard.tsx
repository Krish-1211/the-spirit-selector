    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                            {stats.orders_by_status.map((row: any) => (
                                <div key={row.status} className="flex items-center justify-between">
                                    <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${statusColor[row.status] || "bg-gray-500/10 text-gray-400"}`}>{row.status}</span>
                ...
                <div className="bg-[#111] border border-white/5 rounded-lg p-6">
                    <h2 className="text-sm font-semibold uppercase tracking-widest text-yellow-500/70 mb-4 flex items-center gap-2"><AlertTriangle size={14} /> Low Stock Alerts</h2>
                    {lowStock.length ? (
                        <div className="space-y-3 max-h-64 overflow-auto">
                            {lowStock.map((item: any) => (
                                <div key={item.id} className="flex items-center justify-between text-sm border-b border-white/5 pb-2">
                                    <div><p className="text-white font-medium">{item.product_name}</p><p className="text-gray-500 text-xs">{item.store_name}</p></div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="text-xs uppercase tracking-widest text-gray-600 border-b border-white/5">
                                {["Order #", "Store", "Total", "Status", "Date"].map((h) => <th key={h} className="pb-3 text-left">{h}</th>)}
                            </tr>
                        </thead>
